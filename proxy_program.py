...
import atexit
import signal
listen_PORT = 4500    # pyprox listening to 127.0.0.1:listen_PORT -- 8585?
stop_event = threading.Event()
num_fragment = 30  # total number of chunks that ClientHello devided into (chunks with random size)
...
class DNS_over_Fragment:
    ...
    def query(self,server_name):     
        ...
        if(offline_ip!=None):
            print('offline DNS -->',server_name,offline_ip,file=output_file)
            return offline_ip
        
        cache_ip = DNS_cache.get(server_name,None)
        if(cache_ip!=None):
            print('cached DNS -->',server_name,cache_ip,file=output_file)
            return cache_ip
        ...
        print(f'online DNS Query',server_name,file=output_file)        
        try:
            ...
            if ans.status_code == 200 and ans.headers.get('content-type') == 'application/dns-message':
                ...
                for x in answer_msg.answer:
                    if (x.rdtype == dns.rdatatype.A):
                        ...
                        print("################# DNS Cache is : ####################",file=output_file)
                        print(DNS_cache,file=output_file)         # print DNS cache , it usefull to track all resolved IPs , to be used later.
                        print("#####################################################",file=output_file)
                        break
                
                print(f'online DNS --> Resolved {server_name} to {resolved_ip}',file=output_file)                
                return resolved_ip
            else:
                print(f'Error: {ans.status_code} {ans.reason}',file=output_file)
        except Exception as e:
            print(repr(e),file=output_file)
...
class ThreadedServer(object):
    ...
    def handle_client_request(self,client_socket):
        ...
        if(data[:7]==b'CONNECT'):            
            ...
            print('************************@@@@@@@@@@@@***************************',file=output_file)
            print('redirect',q_method,'http to HTTPS',q_url,file=output_file)          
            response_data = 'HTTP/1.1 302 Found\r\nLocation: '+q_url+'\r\nProxy-agent: MyProxy/1.0\r\n\r\n'            
            client_socket.sendall(response_data.encode())
            client_socket.close()            
            return None
        else:
            print('Unknown Method',str(data[:10]),file=output_file)            
        ...
        print(server_name,'-->',server_port,file=output_file)
        ...
        try:
            ...
            try:
                ...
                print("@@@ "+server_IP+":"+str(server_port)+ " ==> filtered @@@",file=output_file)
                # Send HTTP ERR 502
...
def log_writer():
    ...
    with open(log_file_path, "w") as f:
        while not stop_event.is_set():
            ...
            f.flush()
            f.truncate()
            print("info file writed to",f.name ,file=output_file)
...
def send_data_in_fragment(data , sock):
    ...
    sock.sendall(fragment_data)
    print('----------finish------------',file=output_file)


# def set_proxy(proxy):
#         """
#         Set the proxy settings in Windows Registry
#         """
...
# def unset_proxy():
#     """
#     Unset the proxy settings in Windows Registry
#     """
#     winreg.SetValueEx(INTERNET_SETTINGS, "ProxyEnable", 0, winreg.REG_DWORD, 0)
#     winreg.SetValueEx(INTERNET_SETTINGS, "ProxyServer", 0, winreg.REG_SZ, '')

# def exit_handler():
#     print('Exiting program: clearing proxy...')
#     unset_proxy()

# def signal_handler(signum, frame):
#     print('Closing program: executing atexit functions...')
#     exit_handler()
#     exit(signum)
